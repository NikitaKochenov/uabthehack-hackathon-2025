...
def filename_to_timestamp(filename: str) -> pd.Timestamp:
    match = re.search(r'(\d{4}-\d{2}-\d{2}T\d{2}_\d{2}_\d{2}[+-]\d{2}_\d{2})', filename)
    if match:
        dt_str = match.group(1)
        dt_str = dt_str.replace('_', ':', 2)  # horas:minutos:segundos
        dt_str = dt_str.replace('_', '')      # offset
...
def load_multiple_files(
    ...
    for i, file in enumerate(files):
        try:
            #if verbose:
                #print(f"\n➡️ Procesando archivo {i + 1}/{len(files)}: {file.name}")
            records = load_json_file(file)
            file_ts = filename_to_timestamp(file.name)
            for r in records:
                r["file_timestamp"] = file_ts  # nombre del archivo
